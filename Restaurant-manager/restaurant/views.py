...
def client_reservation(request):
    if request.method == "POST":
        try:
            ...
            if not client:
                max_client_id = Client.objects.aggregate(max_id=Max("id_client"))["max_id"] or 0
                client = Client.objects.create(
                    id_client=max_client_id + 1,
                    nom=nom,
                    tel=tel,
                    type_de_client="Régulier"
                )
                
            with transaction.atomic():
                # Create Commande (Reservation/En ligne)
                max_cmd_id = Commande.objects.aggregate(max_id=Max("id_commande"))["max_id"] or 0
                new_cmd_id = max_cmd_id + 1
                ...
                commande = Commande.objects.create(
                    ...
                    client=client,
                    date=timezone.now(),
                    type_commande="En ligne",
                    montant_total=total_amount,
                    mode_de_paiement="À la livraison"
                )
                ...
                # Deduct stock
                from .models import CompositionProduit, VariationStock
                compositions = CompositionProduit.objects.filter(produit=product)
                for comp in compositions:
                    if comp.quantite_utilisee and comp.ingredient and hasattr(comp.ingredient, 'stock'):
            ...
            return render(request, "restaurant/client_reservation_success.html", {"commande": commande})
            
        except Exception as e:
            return render(request, "restaurant/client_reservation.html", {"error": str(e)})
    ...
    return render(request, "restaurant/client_reservation.html", {"products": products_list})
